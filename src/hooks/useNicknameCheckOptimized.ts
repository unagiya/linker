/**
 * 最適化されたニックネーム利用可能性チェック機能を提供するカスタムフック
 * キャッシュとレート制限を統合したパフォーマンス最適化版
 */

import { useState, useEffect, useCallback } from 'react';
import { validateNickname } from '../utils/nicknameValidation';
import { checkNicknameAvailability } from '../services/nicknameServiceOptimized';
 ...
 * ニックネームチェックの状態
 */
export type NicknameCheckStatus = 'idle' | 'checking' | 'available' | 'unavailable' | 'error' | 'rate_limited';
...
export interface NicknameCheckResult {
  ...
  isValid: boolean;
  isAvailable: boolean;
  retryAfter?: number; // レート制限時の待機時間（ミリ秒）
}
 ...
 * @returns チェック結果と制御関数
 */
export function useNicknameCheckOptimized(
  nickname: string,
